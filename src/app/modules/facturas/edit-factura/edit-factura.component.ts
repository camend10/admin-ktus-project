...
@Component({
  ...
  styleUrl: './edit-factura.component.scss'
})
export class EditFacturaComponent implements OnInit, OnDestroy {
  ...
  showFactura(id: number) {
    this.facturaService.showFactura(id).subscribe((resp) => {
      ...
      if (resp.message === 403) {
        ...
        this.detalle_factura = this.factura.detalles ?? [];
        ...
        this.banco_id = this.factura.factura_pago?.[0]?.banco?.id ?? 9999999;
        ...
        this.municipio_id_deliverie = this.factura.factura_deliverie?.municipio_id ?? 9999999;
        this.direccion_deliverie = this.factura.factura_deliverie?.direccion ?? '';
        this.fecha_deliverie = this.factura.factura_deliverie?.fecha_entrega ?? '';
        this.agencia_deliverie = this.factura.factura_deliverie?.agencia ?? '';
        this.encargado_deliverie = this.factura.factura_deliverie?.encargado ?? '';
        this.documento_deliverie = this.factura.factura_deliverie?.documento ?? '';
        this.celular_deliverie = this.factura.factura_deliverie?.celular ?? '';

        // Busca la sede seleccionada
        const selectedSede = this.sede_deliveries.find(
          (item) => item.id === this.sede_deliverie_id
        );
