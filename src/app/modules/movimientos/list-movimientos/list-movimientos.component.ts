...
@Component({
  ...
  styleUrl: './list-movimientos.component.scss'
})
export class ListMovimientosComponent implements OnInit, OnDestroy {
  ...
  listar(page = 1) {
    ...
    this.movimientoService.listar(page, data).subscribe((resp) => {

      if (resp && resp.movimientos && resp.movimientos.data) {
        this.movimientos = resp.movimientos.data.map((movimiento) => ({
          ...movimiento,
  ...
  cambiarEstado(movimiento: Movimiento) {
    const modalRef = this.modalService.open(DeleteMovimientoComponent, { centered: true, size: 'md' });
