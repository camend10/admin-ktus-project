...
@Component({
  ...
  styleUrl: './create-sede.component.scss'
})
export class CreateSedeComponent implements OnInit {

  isLoading$: any;
  @Output() SedeC: EventEmitter<Sede> = new EventEmitter;
  ...
  store() {
    ...
    this.sedeService.registrar(this.sede).subscribe((resp) => {
      if (resp.message === 403) {
        this.toast.error('Validación', resp.message_text);
      } else {
        this.toast.success('Exito', resp.message_text);
        this.SedeC.emit(resp.sede);
        this.modal.close();
      }
    });
