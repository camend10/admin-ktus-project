...
import { Departamento, Genero, Municipio, TipoDoc, Role, Empresa, ResponseConfiguracion, ResponseConfiguracionArticulos, ResponseConfiguracionConversion } from '../interfaces';
...
export class GeneralesService {
  ...
  cargarConfiguracionesConversiones(empresa_id: number): Observable<ResponseConfiguracionConversion> {
    this.isLoadingSubject.next(true);
    let url = URL_SERVICIOS + '/generales/configuraciones/conversiones';

    let data = {
      empresa_id: empresa_id
    };

    return this.http.post<ResponseConfiguracionConversion>(url, data)
      .pipe(
        finalize(() => this.isLoadingSubject.next(false))
      );
  }
